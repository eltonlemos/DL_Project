...
class Car:
    def __init__(self) -> None:
        ...
        self.i=1
        self.epsilon=1
        self.actor = ActorNet([1080,512,256],1)
        self.actor_target = copy.deepcopy(self.actor)
        self.critic = CriticNet([1080,512,256],1)
    ...
    def replay_exp(self):
        ...
        states_next = torch.stack(states_next).squeeze(1)
        ...
        target_Q = self.critic_target(states, target_actions).squeeze(1)
        target_Q = rewards + ((1 - terminated)*(1 - truncated) * self.discount * target_Q).detach()
    ...
    for i in range(episodes):
        ...
        while not (terminated or truncated):
            ...
            if len(agent.D)<=(agent.len_d-1):
                    agent.D.append([obs,action,reward,agent.obs.copy(),terminated,truncated])
            else:
                    agent.D.pop(0)
                    agent.D.append([obs,action,reward,agent.obs.copy(),terminated,truncated])
            obs = agent.obs
...
if __name__ == "__main__":
    ...
    with open('race_car_ddpg_actor.pickle', 'wb') as f:
        pickle.dump(agent.actor, f)
    with open('race_car_ddpg_critic.pickle', 'wb') as f:
